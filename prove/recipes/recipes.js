...
// 🔹 Single recipe card template (similar to your Apple Crisp HTML)
function recipeTemplate(recipe) {
  // fallback link if url is empty
  const href = recipe.url && recipe.url.trim() !== "" ? recipe.url : "#";

  // optional meta line
  const metaBits = [];
  if (recipe.prepTime) metaBits.push(`Prep: ${recipe.prepTime}`);
  if (recipe.cookTime) metaBits.push(`Cook: ${recipe.cookTime}`);
  if (recipe.recipeYield) metaBits.push(`Yield: ${recipe.recipeYield}`);
  const meta = metaBits.length ? `<p class="recipe-meta">${metaBits.join(" | ")}</p>` : "";
  ...
  return `
    <article class="recipe-card">
      ...
      <div class="recipe-info">
        ...
        <h2><a class="recipe-link" href="${href}">${recipe.name}</a></h2>
...
// 🔹 Show one random recipe on load (like hikes)
function init() {
